...
esp_err_t sdcard_init(void) {
    ...
    ESP_LOGI(TAG,
             "Attempting to mount SD card at %s (SDSPI host slot=%d, "
             "max_freq=%d kHz)...",
    ...
    esp_vfs_fat_sdmmc_mount_config_t mount_config = {
        .format_if_mount_failed =
            false, // set true if you want auto-format on first failure
        .max_files = 5,
    ...
    ret = esp_vfs_fat_sdspi_mount(SDCARD_MOUNT_POINT, &host, &slot_config,
                                  &mount_config, &s_card);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to mount SD card: %s (0x%x)",
                 esp_err_to_name(ret), ret);
        ESP_LOGE(TAG, "Troubleshooting tips:");
        ESP_LOGE(TAG,
                 "  1. Check wiring: MOSI->GPIO%d, MISO->GPIO%d, SCK->GPIO%d, "
                 "CS->GPIO%d",
                 SDCARD_PIN_MOSI, SDCARD_PIN_MISO, SDCARD_PIN_SCK,
                 SDCARD_PIN_CS);
        ESP_LOGE(
            TAG,
            "  2. Verify power: Ensure SD module has stable 3.3V or 5V power");
        ESP_LOGE(TAG, "  3. Check ground: Ensure common ground between ESP32 "
                      "and SD module");
        ESP_LOGE(TAG, "  4. Verify card: Ensure SD card is inserted and "
                      "formatted as FAT32");
        ESP_LOGE(
            TAG,
            "  5. Try slower: If issue persists, reduce max_freq_khz in code");
...
/**
 * @brief Test SD card by reading capacity and performing a simple read/write
 * test
 *
 * This function:
...
esp_err_t sdcard_test_read(void) {
    ...
    if (card_size_gb < 28 || card_size_gb > 35) {
        ESP_LOGW(TAG,
                 "WARNING: Card size (%.2f GB) doesn't match expected 32GB",
                 (double)card_size_gb);
    } else {
    ...
    ESP_LOGI(TAG, "✓ Test file written successfully (%d bytes)",
             strlen(test_data));
